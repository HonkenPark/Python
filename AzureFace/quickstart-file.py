...
if not detected_faces:
	raise Exception('[ERROR] No face detected from image {}'.format(single_image_name))

# Display the detected face ID in the first single-face image.
# Face IDs are used for comparison to faces (their IDs) detected in other images.
print('Detected face ID from [', single_image_name, '] :')
for face in detected_faces: print ('face_id [', face.face_id, ']')
print()
...
# We use detection model 3 to get better performance.
detected_faces_multiple = face_client.face.detect_with_url(url=multi_face_image_url, detection_model='detection_03')
# </snippet_detectgroup>

print('Detected face IDs from [', multi_image_name, '] :')
if not detected_faces_multiple:
	raise Exception('[ERROR] No face detected from image {}.'.format(multi_image_name))
else:
    for face in detected_faces_multiple:
...
single_image_name = os.path.basename(single_face_image_url)
# We use detection model 3 to get better performance.
detected_face_url = face_client.face.detect_with_url(url=single_face_image_url, detection_model='detection_03')

#for file stream
img_path = 'D:\\Other\\Python\\AzureFace\\single_face_heungmin.jpg'
with open(img_path, mode="rb") as single_face_image_stream:
    detected_face_stream = face_client.face.detect_with_stream(image=single_face_image_stream, return_face_id=True, return_face_attributes=['age', 'gender', 'emotion'])

print('Detected face ID from [', single_image_name, '] :')
if not detected_face_stream:
	raise Exception('[ERROR] No face detected from image {}'.format(single_image_name))
else:
    for face2 in detected_face_stream: print ('face_id2 [', face2.face_id, ']')
print()
...
# Download the image from the url
    response = requests.get(single_face_image_stream)
    img = Image.open(BytesIO(response.content))

# For each face returned use the face rectangle and draw a red box.
    print('Drawing rectangle around face... see popup for results.')
    draw = ImageDraw.Draw(img)
    for face in detected_face_stream:
...
print('Detect Face Test is finished !!')
'''
END - Detect faces
'''
...
second_image_face_IDs = list(map(lambda x: x.face_id, detected_faces_multiple))
...
# Print the details of the similar faces detected
	...
	for face in similar_faces:
		...
		face_info = next(x for x in detected_faces_multiple if x.face_id == first_image_face_ID)
		if face_info:
...
detected_faces_multiple = face_client.face.detect_with_url(IMAGE_BASE_URL + source_image_file_name2, detection_model='detection_03')
# Add the returned face's face ID
source_image2_id = detected_faces_multiple[0].face_id
print('{} face(s) detected from image {}.'.format(len(detected_faces_multiple), source_image_file_name2))
